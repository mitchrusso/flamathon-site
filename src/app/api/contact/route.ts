...
const fromEmail = process.env.CONTACT_FROM_EMAIL || "Flamathon <onboarding@resend.dev>";
...
${message}`) > MAX_URLS) {
  ...
  const { error } = await resend.emails.send({
    ...
    subject: `Flamathon contact: ${subject}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #18211f;">
        <h2>New Flamathon Contact Form Message</h2>
    ...
    `,
    text: `New Flamathon Contact Form Message\n\nName: ${name}\nEmail: ${email}\nSubject: ${subject}\n\n${message}`,
  });
