...
function scoreProductForArticle(article: ResourceArticle, product: ReviewProduct) {
  ...
  if (articleText.includes("challenge") && productText.includes("challenge")) score += 8;
  if (articleText.includes("ghost") && productText.includes("ghost")) score += 8;
  if (articleText.includes("reaper") && productText.includes("reaper")) score += 8;
  if (articleText.includes("ramen") && productText.includes("ramen")) score += 8;
  if (articleText.includes("chili crisp") && productText.includes("chili crisp")) score += 8;
  if (articleText.includes("bbq") && productText.includes("bbq")) score += 7;
  if (articleText.includes("wing") && productText.includes("wing")) score += 7;
  if (articleText.includes("tasting") && productText.includes("tasting")) score += 6;
  ...
  return [
    {
      ...
      answer: "Compare the ingredients, allergens, heat level, serving size, seller details, and storage instructions, and whether the product matches your group and serving setup.",
    },
    {
    ...
    },
    {
      ...
      answer: "Confirm the product category fits your guest preferences, label needs, heat tolerance, and serving plan, then check manufacturer instructions and current labels and source references before purchase.",
    },
  ];
}
...
export async function generateMetadata({ params }: ArticlePageProps) {
  ...
  if (!article || !isArticlePublished(article)) {
    return {
      title: "Resource Not Found | Flamathon",
    };
  }
  ...
  const articleJsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        name: `Recommended Flamathon shopping guides for ${article.title}`,
